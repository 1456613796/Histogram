...
groupSize = 16
localId = 0-15
globalId = 1024*768/256 = 3072
groupId = globalId/groupSize
*/
...
"    for(int i = 0; i < BIN_SIZE; ++i)                                                 \n"
"    {                                                                                 \n"
"        uint value = data[groupId*groupSize*BIN_SIZE + localId + i * groupSize];                                   \n"
...
// 计算可得GlobbalThreads = 3072
// 本机wrapSize=32

int main(int argc, char *argv[])
{
	cl_int status = 0;
	cl_int binSize = 256;//每个工作项的处理的数据数目
	cl_int groupSize = 96;//每个工作组的工作项数
	cl_int subHistgCnt;
	...
	//(image->width * image->height) / binSize得到的是工作项的数目，在除以groupSize得到的就是工作组的数目
	//binSize * subHistgCnt
	subHistgCnt = (image->width * image->height) / (binSize*groupSize);//工作组的数目
	midDeviceBinR = (cl_uint*)malloc(binSize * subHistgCnt * sizeof(cl_uint));//分配中间结果的存储空间
	midDeviceBinG = (cl_uint*)malloc(binSize * subHistgCnt * sizeof(cl_uint));//全局变量，用来存储结果值
	midDeviceBinB = (cl_uint*)malloc(binSize * subHistgCnt * sizeof(cl_uint));//全局变量
	...
	cl_event ndrEvt;
	size_t globalThreads = (image->width * image->height) / (binSize*groupSize) * groupSize;//总线程的个数
	size_t localThreads = groupSize;//局部线程的个数
	...
	status = clFinish(commandQueue);//执行结束
	...
	cout << "内核执行时间是：" << kernelExecTimeNs << endl;
